...
class Dataset():
    ...
    def process_data(self, data):
        sentences, labels = [], []
        # print(len(data))
        for label, sentence in zip(list(data['label']), list(data['translated'])):
...
try:
    with open('bert_arabic.pkl', 'rb') as f:
        data_dict = pickle.load(f)
except:
    data_dict = {}
    with open('bert_arabic.pkl', 'wb') as f:
...
for random_seed in random_seeds:
    ...
    for attn in attentions:
        ...
        model.to(device)
        if attn!=0:
            model.load_state_dict(torch.load('./Saved_bert/bert-base-uncased_11_6_2_'+str(attn)+'/pytorch_model.bin', 'cpu'))
        else:
            model.load_state_dict(torch.load('./Saved_bert/bert-base-uncased__2/pytorch_model.bin', 'cpu'))
        model.to(device)
        if n!=0:
